...
const B4XPP_GENERATOR_VERSION = '0.3.1';
  ...
  for (const [lname, group] of methodGroups) {
    ...
    group.forEach((sig, idx) => {
      ...
      if (byArity.has(arity)) {
        diagnostics && diagnostics.push(makeDiagnostic(sourcePath, sig.absoluteLine, 'error', `Ambiguous overload: ${ownerName}.${sig.name} has more than one overload with ${arity} parameter(s). v0.3.1 resolves overloads by parameter count only.`));
...
function transformOopLine(rawLine, context) {
  ...
  if (constructorStart) {
    const generatedName = context.overloadPlan && context.overloadPlan.constructorNamesByLine && context.overloadPlan.constructorNamesByLine.get(context.lineNo) || 'Initialize';
  ...
  if (methodSig) {
    ...
    if (methodSig.modifiers.includes('abstract')) {
      ...
      const generatedName = context.overloadPlan && context.overloadPlan.methodNamesByLine && context.overloadPlan.methodNamesByLine.get(context.lineNo) || methodSig.name;
    ...
    } else if (context.overloadPlan && context.overloadPlan.methodNamesByLine && context.overloadPlan.methodNamesByLine.has(context.lineNo)) {
      const generatedName = context.overloadPlan.methodNamesByLine.get(context.lineNo);
