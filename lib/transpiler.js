...
const B4XPP_GENERATOR_VERSION = '0.1';
...
function parseMethodSignatureLine(line) {
  const m = line.match(/^\s*((?:(?:Override|Virtual|Protected|Abstract|Final)\s+)*)?(?:(Public|Private)\s+)?Sub\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?:\(([^)]*)\))?\s*(?:As\s+(.+?))?\s*$/i);
  if (!m) return null;
  const modifiers = (m[1] || '').trim().split(/\s+/).filter(Boolean).map(s => s.toLowerCase());
  const visibility = (m[2] || '').trim().toLowerCase() || null;
  const paramsRaw = m[4] || '';
...
function transformOopLine(rawLine, context) {
  ...
  if (constructorStart) {
    return [`${constructorStart[1]}Public Sub Initialize(${constructorStart[2] || ''})`];
  }
  ...
  if (/^\s*Protected\s+(?!Sub\b)/i.test(line)) {
    line = line.replace(/^(\s*)Protected\s+/i, '$1Private ');
  }

  const leadingMatch = line.match(/^(\s*)((?:(?:Override|Virtual|Protected|Abstract|Final)\s+)+)(.*)$/i);
  if (leadingMatch) {
    const indent = leadingMatch[1];
    const modifiers = leadingMatch[2].trim().split(/\s+/).map(m => m.toLowerCase());
    let rest = leadingMatch[3];
    comments.push(`${indent}' B4X++ modifiers: ${modifiers.join(', ')}`);

    if (modifiers.includes('abstract') && /^Sub\b/i.test(rest)) {
      return comments.concat(makeAbstractStub(indent, rest, context));
    }

    if (/^Sub\b/i.test(rest)) {
      rest = `Public ${rest}`;
    } else if (/^Private\s+Sub\b/i.test(rest) && modifiers.includes('protected')) {
      rest = rest.replace(/^Private\s+Sub\b/i, 'Public Sub');
    }
    line = indent + rest;
  }
...
function replaceSuperMembers(line, context) {
  ...
  code = code.replace(/\bSuper\.([A-Za-z_][A-Za-z0-9_]*)/g, (full, member, offset, whole) => {
    ...
    return context.resolveSuperMember(member, hasCall);
  });
  code = code.replace(/\bsuper\.([A-Za-z_][A-Za-z0-9_]*)/g, (full, member, offset, whole) => {
    let i = offset + full.length;
    while (i < whole.length && /\s/.test(whole[i])) i++;
...
function makeAbstractStub(indent, rest, context) {
  const signature = `${indent}Public ${rest}`;
  const sig = parseMethodSignatureLine(signature);
...
function classLineBundle(block, diagnostics) {
  ...
  return {
    ...
    fields: propertyExpansion.fields.concat(globals.fields).map(transformClassGlobalFieldLine),
...
function flattenClassParts(block, options, programInfo, diagnostics, usesRuntime) {
  ...
  for (let idx = 0; idx < chain.length; idx++) {
    ...
    const ownerFieldNames = collectFieldNames(bundle.fields);
    const fields = [];
    for (const fieldLine of bundle.fields) {
    ...
    for (let i = 0; i < bundle.lines.length; i++) {
      const context = {
        ...
        flattenSuper: true,
        resolveSuperMember: (memberName, hasCall) => {
          ...
          const methodOwner = getParentMethodOwner(programInfo, parentName, memberName) || parentName;
          return makeSuperMethodName(methodOwner, memberName);
        }
      };
    ...
    for (const line of transformed) {
      ...
      const shouldRenameToSuper = !isFinalBlock && !isSpecial && (lname === 'initialize' || descendantHasMethod(sig.name));
...
function sanitizeMethodParameterCollisions(body, globalNames, diagnostics, sourcePath, baseLine) {
  if (!globalNames || globalNames.size === 0) return body;
  ...
  for (let i = 0; i < out.length; i++) {
    ...
    for (const param of sig.params) {
      ...
      if (!lname || !globalNames.has(lname)) continue;
      const safeName = makeSafeParameterName(param.name, globalNames);
      replacements.set(lname, safeName);
      if (diagnostics) {
        diagnostics.push(makeDiagnostic(sourcePath || '', (baseLine || 0) + i + 1, 'warning', `Parameter ${param.name} in ${sig.name} conflicts with a generated global field and was renamed to ${safeName}.`));
      }
    }
...
function extractPropertyDirectives(lines, block, diagnostics) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    properties.push({ name, fieldName, type, mode });
    ...
    accessors.push('');
    accessors.push(`${indent}' B4X++ property: ${name} As ${type}${mode ? ' (' + mode + ')' : ''}${defaultValue ? ' default=' + defaultValue : ''}`);
    if (mode !== 'writeonly') {
      accessors.push(`${indent}Public Sub get${name} As ${type}`);
    ...
    if (mode !== 'readonly') {
      ...
      accessors.push(`${indent}Public Sub set${name}(${setterParamName} As ${type})`);
...
function parsePropertyDirective(line) {
  ...
  const m = code.match(/^#Property\s+(?:(ReadOnly|WriteOnly)\s+)?([A-Za-z_][A-Za-z0-9_]*)\s+As\s+(.+)$/i);
  if (!m) return null;

  const mode = (m[1] || '').toLowerCase();
  const name = m[2];
  let typeAndDefault = (m[3] || '').trim();
  ...
  return { indent, mode, name, type, defaultValue };
}
