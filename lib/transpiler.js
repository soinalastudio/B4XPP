...
const B4XPP_GENERATOR_VERSION = '0.3.5';

const IDENTIFIER_RE = /^[A-Za-z_][A-Za-z0-9_]*$/;
const B4X_RESERVED_WORDS = new Set([
  'as','case','class','dim','do','each','else','end','false','for','if','loop','next','not','null','private','public','return','select','step','sub','then','to','true','until','while'
]);
...
function parseProjectDirectives(sourcePath, lines, diagnostics) {
  const project = { dependsOn: [], b4aDependsOn: [], b4jDependsOn: [], b4iDependsOn: [] };
  for (let index = 0; index < lines.length; index++) {
    ...
    if (dependsMatch) {
      project.dependsOn.push(...splitCsvDirective(dependsMatch[1]));
      continue;
    }

    const b4aDependsMatch = trimmed.match(/^#B4ADependsOn\s+(.+)$/i);
    if (b4aDependsMatch) {
      project.b4aDependsOn.push(...splitCsvDirective(b4aDependsMatch[1]));
      continue;
    }

    const b4jDependsMatch = trimmed.match(/^#B4JDependsOn\s+(.+)$/i);
    if (b4jDependsMatch) {
      project.b4jDependsOn.push(...splitCsvDirective(b4jDependsMatch[1]));
      continue;
    }

    const b4iDependsMatch = trimmed.match(/^#B4iDependsOn\s+(.+)$/i);
    if (b4iDependsMatch) {
      project.b4iDependsOn.push(...splitCsvDirective(b4iDependsMatch[1]));
      continue;
    }
  ...
  project.dependsOn = uniqueStrings(project.dependsOn);
  project.b4aDependsOn = uniqueStrings(project.b4aDependsOn);
  project.b4jDependsOn = uniqueStrings(project.b4jDependsOn);
  project.b4iDependsOn = uniqueStrings(project.b4iDependsOn);
