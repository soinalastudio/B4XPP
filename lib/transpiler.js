...
const B4XPP_GENERATOR_VERSION = '0.5.8';
...
const B4X_RESERVED_WORDS = new Set([
  'as','async','await','case','class','closure','dim','do','each','else','end','false','for','if','loop','next','not','null','private','public','return','select','step','sub','then','to','true','until','while'
]);
...
function parseClassDirective(rest) {
  const result = { name: null, extendsName: null, implementsNames: [], modifiers: [], genericParams: [] };
  ...
  if (/^\(\s*Of\b/i.test(text)) {
    const parsed = readBalancedParentheses(text, 0);
    if (parsed) {
      const inner = String(parsed.inner || '').trim();
      const ofMatch = inner.match(/^Of\s+([\s\S]+)$/i);
      if (ofMatch) {
        result.genericParams = splitArguments(ofMatch[1]).map(s => s.trim()).filter(Boolean);
      }
      text = text.slice(parsed.end + 1).trimStart();
    }
  }
...
function parseProjectDirectives(sourcePath, lines, diagnostics) {
  const project = {
    ...
    b4xppLibDependsOn: [], b4xppLibB4ADependsOn: [], b4xppLibB4JDependsOn: [], b4xppLibB4iDependsOn: [], b4xppLibSupportedPlatforms: [],
    supportedPlatforms: [], b4xLibName: '', b4xLibVersion: '', b4xLibAuthor: '', b4xppLibName: '', b4xppLibVersion: '', b4xppLibAuthor: '',
    banano: {}
  };
  for (let index = 0; index < lines.length; index++) {
    ...
    if (projectMatch) {
      ...
      if (!platform) {
        diagnostics.push(makeDiagnostic(sourcePath, lineNo, 'error', `Unknown #Project platform: ${parts[0] || ''}. Use BANano, B4J-NonUI, B4J-UI, B4A or B4i.`));
        continue;
      }
    ...
    const bananoMatch = trimmed.match(/^#BANano([A-Za-z0-9_]*)\b(?:\s+(.+))?$/i);
    if (bananoMatch) {
      const rawKey = bananoMatch[1] || 'Enabled';
      const key = rawKey.charAt(0).toLowerCase() + rawKey.slice(1);
      const rawValue = (bananoMatch[2] || 'True').trim().replace(/^["']|["']$/g, '');
      project.banano[key] = rawValue;
      project.sourcePath = project.sourcePath || sourcePath;
      project.line = project.line || lineNo;
      continue;
    }
    ...
    const b4xppLibMatch = trimmed.match(/^#B4XPPLib\s+(.+)$/i);
    if (b4xppLibMatch) { project.b4xppLibName = b4xppLibMatch[1].trim().replace(/^["']|["']$/g, ''); project.sourcePath = project.sourcePath || sourcePath; project.line = project.line || lineNo; continue; }
    const b4xppLibVersionMatch = trimmed.match(/^#B4XPPLibVersion\s+(.+)$/i);
    if (b4xppLibVersionMatch) { project.b4xppLibVersion = b4xppLibVersionMatch[1].trim().replace(/^["']|["']$/g, ''); continue; }
    const b4xppLibAuthorMatch = trimmed.match(/^#B4XPPLibAuthor\s+(.+)$/i);
    if (b4xppLibAuthorMatch) { project.b4xppLibAuthor = b4xppLibAuthorMatch[1].trim().replace(/^["']|["']$/g, ''); continue; }
    const b4xppLibSupportedPlatformsMatch = trimmed.match(/^#B4XPPLibSupportedPlatforms\s+(.+)$/i);
    if (b4xppLibSupportedPlatformsMatch) { project.b4xppLibSupportedPlatforms.push(...splitCsvDirective(b4xppLibSupportedPlatformsMatch[1])); project.supportedPlatforms.push(...splitCsvDirective(b4xppLibSupportedPlatformsMatch[1])); project.sourcePath = project.sourcePath || sourcePath; project.line = project.line || lineNo; continue; }
    const b4xppLibDependsMatch = trimmed.match(/^#B4XPPLibDependsOn\s+(.+)$/i);
    if (b4xppLibDependsMatch) { project.b4xppLibDependsOn.push(...splitCsvDirective(b4xppLibDependsMatch[1])); continue; }
    const b4xppLibB4aDependsMatch = trimmed.match(/^#B4XPPLibB4ADependsOn\s+(.+)$/i);
    if (b4xppLibB4aDependsMatch) { project.b4xppLibB4ADependsOn.push(...splitCsvDirective(b4xppLibB4aDependsMatch[1])); continue; }
    const b4xppLibB4jDependsMatch = trimmed.match(/^#B4XPPLibB4JDependsOn\s+(.+)$/i);
    if (b4xppLibB4jDependsMatch) { project.b4xppLibB4JDependsOn.push(...splitCsvDirective(b4xppLibB4jDependsMatch[1])); continue; }
    const b4xppLibB4iDependsMatch = trimmed.match(/^#B4XPPLibB4iDependsOn\s+(.+)$/i);
    if (b4xppLibB4iDependsMatch) { project.b4xppLibB4iDependsOn.push(...splitCsvDirective(b4xppLibB4iDependsMatch[1])); continue; }
  ...
  for (const key of ['dependsOn','b4aDependsOn','b4jDependsOn','b4iDependsOn','b4xLibDependsOn','b4xLibB4ADependsOn','b4xLibB4JDependsOn','b4xLibB4iDependsOn','b4xppLibDependsOn','b4xppLibB4ADependsOn','b4xppLibB4JDependsOn','b4xppLibB4iDependsOn']) project[key] = uniqueStrings(project[key]);
  ...
  project.b4xppLibSupportedPlatforms = uniqueStrings(project.b4xppLibSupportedPlatforms.map(s => String(s || '').toUpperCase().replace('B4I', 'B4i')));
  return (project.platform || project.b4xLibName || project.b4xppLibName || project.supportedPlatforms.length || project.b4xLibSupportedPlatforms.length || project.b4xppLibSupportedPlatforms.length || project.dependsOn.length || project.b4aDependsOn.length || project.b4jDependsOn.length || project.b4iDependsOn.length || project.b4xLibDependsOn.length || project.b4xLibB4ADependsOn.length || project.b4xLibB4JDependsOn.length || project.b4xLibB4iDependsOn.length || project.b4xppLibDependsOn.length || project.b4xppLibB4ADependsOn.length || project.b4xppLibB4JDependsOn.length || project.b4xppLibB4iDependsOn.length) ? project : null;
}
...
function discoverIncludedFiles(files, options = {}) {
  ...
  for (const file of files) {
    const text = readSourceTextForTranspile(file, options);
    expandIncludes(file, text, options, [], included);
  }
  return included;
}

function readSourceTextForTranspile(file, options = {}) {
  const key = path.resolve(file);
  const overrides = options && options.fileTextOverrides;
  if (overrides) {
...
function parseMethodSignatureLine(line) {
  ...
  const m = String(code || '').match(/^\s*((?:(?:Public|Private|Protected|Override|Virtual|Abstract|Final|Async)\s+)*)Sub\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?:\((.*)\))?\s*(?:As\s+(.+?))?\s*$/i);
  if (!m) return null;
...
function parseBxFile(sourcePath, text, options = {}) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (/^#(?:Project|Package|ProjectDir|ProjectDependsOn|ProjectB4ADependsOn|ProjectB4JDependsOn|ProjectB4iDependsOn|BANano[A-Za-z0-9_]*|MobileMain|B4XLib|B4XLibDir|B4XLibVersion|B4XLibAuthor|B4XLibSupportedPlatforms|B4XLibDependsOn|B4XLibB4ADependsOn|B4XLibB4JDependsOn|B4XLibB4iDependsOn|B4XPPLib|B4XPPLibDir|B4XPPLibVersion|B4XPPLibAuthor|B4XPPLibSupportedPlatforms|B4XPPLibDependsOn|B4XPPLibB4ADependsOn|B4XPPLibB4JDependsOn|B4XPPLibB4iDependsOn|LibraryFilesDir|Version|Author|DependsOn|B4ADependsOn|B4JDependsOn|B4iDependsOn|SupportedPlatforms|ShortName)\b/i.test(trimmed)) continue;
    ...
    if (currentInterface) {
      ...
      if (!currentInterface.lineNumbers) currentInterface.lineNumbers = [];
      currentInterface.lineNumbers.push(lineNo);
      continue;
    }
    ...
    if (currentStatic) {
      ...
      if (!currentStatic.lineNumbers) currentStatic.lineNumbers = [];
      currentStatic.lineNumbers.push(lineNo);
      continue;
    }
    ...
    if (classMatch) {
      ...
      const genericParams = (parsed.genericParams || []).map(p => sanitizeModuleName(p)).filter(Boolean);
      if ((parsed.genericParams || []).length !== genericParams.length) {
        diagnostics.push(makeDiagnostic(sourcePath, lineNo, 'error', `Invalid generic parameter in Class ${className}. Use identifiers such as T, TKey, TValue.`));
      }
      current = {
        type: 'class',
        name: className,
    ...
    if (current) {
      ...
      if (!current.lineNumbers) current.lineNumbers = [];
      current.lineNumbers.push(lineNo);
    } else {
      topLevelLines.push(rawLine);
      topLevelLineNumbers.push(lineNo);
    }
  }
...
const B4XPP_BUILTIN_TYPES = new Set([
  ...
  'string','int','long','float','double','boolean','object','list','map','resumablesub','sub','closure','b4xpp_closure'
]);
...
function normalizePlatformKey(value) {
  ...
  if (platform.includes('banano')) return 'b4j';
...
function projectPlatformKeys(project, options) {
  ...
  const supportedRaw = project ? (
    (project.b4xppLibSupportedPlatforms && project.b4xppLibSupportedPlatforms.length) ? project.b4xppLibSupportedPlatforms :
    (project.b4xLibSupportedPlatforms && project.b4xLibSupportedPlatforms.length) ? project.b4xLibSupportedPlatforms :
    (project.supportedPlatforms || [])
  ) : [];
  const supported = uniqueStrings(supportedRaw
    .map(normalizePlatformKey)
    .filter(Boolean));
  if (supported.length) return supported;

  if (project && (project.b4xLibName || project.b4xppLibName)) return ['b4a', 'b4j', 'b4i'];
  return ['b4j', 'b4a', 'b4i'];
}
...
function collectLibraryDirs(options, project) {
  ...
  dirs.push(...readArrayOption(options || {}, 'b4xppBundledLibraryDirs', 'b4xpplibBundledLibraryDirs'));
...
function parseKeyValueManifest(text) {
  const values = {};
  for (const raw of splitLines(stripUtf8Bom(text || ''))) {
    const line = raw.trim();
...
function readZipTextEntries(entries, extensionRe) {
  const out = new Map();
  for (const entry of entries || []) {
    const cleanName = String(entry.name || '').replace(/\\/g, '/');
    if (!extensionRe.test(cleanName) || /(^|\/)\./.test(cleanName) || /^Files\//i.test(cleanName)) continue;
    out.set(cleanName, stripUtf8Bom(entry.data.toString('utf8')));
  }
  return out;
}
...
function resolveZipIncludeEntry(currentEntry, target, entriesByLower) {
  const cleanTarget = String(target || '').replace(/^['"]|['"]$/g, '').replace(/\\/g, '/');
  const currentDir = String(currentEntry || '').replace(/[^/]*$/, '');
  const candidates = [cleanTarget, normalizeZipPath(currentDir + cleanTarget)];
  const withBx = [];
  for (const c of candidates) {
    withBx.push(c);
    if (!/\.bx$/i.test(c)) withBx.push(c + '.bx');
  }
  for (const c of withBx) {
    const key = normalizeZipPath(c).toLowerCase();
    if (entriesByLower.has(key)) return entriesByLower.get(key).name;
  }
  return '';
}

function expandB4XPPLibIncludes(packagePath, entryName, textMap, stack = [], includeCollector = new Set()) {
  const diagnostics = [];
  const entriesByLower = new Map();
  for (const [name, text] of textMap.entries()) entriesByLower.set(String(name).toLowerCase(), { name, text });
  const normalizedEntry = String(entryName || '').replace(/\\/g, '/');
  if (stack.map(s => s.toLowerCase()).includes(normalizedEntry.toLowerCase())) {
    return { text: textMap.get(entryName) || '', diagnostics: [makeDiagnostic(`${packagePath}::${entryName}`, 1, 'error', `Circular #Include inside B4XPPLib: ${entryName}.`)], includedFiles: includeCollector };
  }
  const output = [];
  const lines = splitLines(textMap.get(entryName) || '');
  for (let i = 0; i < lines.length; i++) {
    const raw = lines[i];
    const target = parseIncludeTarget(raw.trim());
    if (!target) { output.push(raw); continue; }
    const resolved = resolveZipIncludeEntry(normalizedEntry, target, entriesByLower);
    if (!resolved) {
      diagnostics.push(makeDiagnostic(`${packagePath}::${entryName}`, i + 1, 'error', `Fichier inclus introuvable dans le B4XPPLib : ${target}.`));
      continue;
    }
    includeCollector.add(resolved);
    const expanded = expandB4XPPLibIncludes(packagePath, resolved, textMap, stack.concat([normalizedEntry]), includeCollector);
    diagnostics.push(...expanded.diagnostics);
    output.push(`' B4X++ package include begin: ${resolved}`);
    output.push(...splitLines(expanded.text));
    output.push(`' B4X++ package include end: ${resolved}`);
  }
  return { text: output.join('\n'), diagnostics, includedFiles: includeCollector };
}

function parseB4XPPLibFile(b4xpplibPath) {
  const entries = readZipEntries(b4xpplibPath);
  const manifestEntry = entries.find(e => /(^|\/)manifest\.txt$/i.test(e.name));
  const manifestText = manifestEntry ? stripUtf8Bom(manifestEntry.data.toString('utf8')) : '';
  const manifest = parseKeyValueManifest(manifestText);
  const libraryName = manifest.name || path.basename(b4xpplibPath, '.b4xpplib');
  const version = manifest.version || '';
  const sourceTexts = readZipTextEntries(entries, /\.bx$/i);
  const included = new Set();
  const parsedFiles = [];
  const diagnostics = [];

  for (const [entryName, text] of sourceTexts.entries()) {
    if (included.has(entryName)) continue;
    const expanded = expandB4XPPLibIncludes(b4xpplibPath, entryName, sourceTexts, [], included);
    diagnostics.push(...expanded.diagnostics);
    const parsed = parseBxFile(`${b4xpplibPath}::${entryName}`, expanded.text);
    parsed.fromB4XPPLib = true;
    parsed.b4xpplibPath = b4xpplibPath;
    parsed.b4xpplibName = libraryName;
    parsed.libraryProject = parsed.project || null;
    parsed.project = null; // Do not let package metadata become the consuming project's #Project.
  ...
  const projects = [];
  for (const parsed of parsedFiles) {
    if (parsed.libraryProject) projects.push(parsed.libraryProject);
    else if (parsed.project) projects.push(parsed.project);
    for (const cls of parsed.classes || []) {
      classes.push({
        name: cls.name,
        shortName: cls.name,
        fullName: `${libraryName}.${cls.name}`,
        kind: 'Class',
        genericParams: (cls.genericParams || []).slice(),
        methods: (cls.methods || []).filter(m => String(m.visibility || 'public').toLowerCase() !== 'private').map(m => ({ ...m, source: cls.sourcePath, libraryName })),
        libraryPath: b4xpplibPath,
        sourcePath: cls.sourcePath
      });
    }
    for (const mod of parsed.staticCodes || []) {
      staticCodes.push(mod);
      classes.push({
        name: mod.name,
        shortName: mod.name,
        fullName: `${libraryName}.${mod.name}`,
        kind: 'StaticCode',
        methods: (mod.methods || []).filter(m => String(m.visibility || 'public').toLowerCase() !== 'private').map(m => ({ ...m, source: mod.sourcePath, libraryName })),
        libraryPath: b4xpplibPath,
        sourcePath: mod.sourcePath
      });
    }
    for (const intf of parsed.interfaces || []) interfaces.push(intf);
  }

  const dependsOn = uniqueStrings([
    ...splitCsvDirective(manifest.b4xpplibdependson || manifest.dependson || ''),
    ...projects.flatMap(p => p.b4xppLibDependsOn || [])
  ]);
  const b4aDependsOn = uniqueStrings([
    ...splitCsvDirective(manifest['b4a.b4xpplibdependson'] || ''),
    ...projects.flatMap(p => p.b4xppLibB4ADependsOn || [])
  ]);
  const b4jDependsOn = uniqueStrings([
    ...splitCsvDirective(manifest['b4j.b4xpplibdependson'] || ''),
    ...projects.flatMap(p => p.b4xppLibB4JDependsOn || [])
  ]);
  const b4iDependsOn = uniqueStrings([
    ...splitCsvDirective(manifest['b4i.b4xpplibdependson'] || ''),
    ...projects.flatMap(p => p.b4xppLibB4iDependsOn || [])
  ]);
  const nativeDependsOn = uniqueStrings([
    ...splitCsvDirective(manifest.projectdependson || manifest.nativedependson || ''),
    ...projects.flatMap(p => p.dependsOn || [])
  ]);
  const nativeB4ADependsOn = uniqueStrings([
    ...splitCsvDirective(manifest.projectb4adependson || manifest['b4a.projectdependson'] || ''),
    ...projects.flatMap(p => p.b4aDependsOn || [])
  ]);
  const nativeB4JDependsOn = uniqueStrings([
    ...splitCsvDirective(manifest.projectb4jdependson || manifest['b4j.projectdependson'] || ''),
    ...projects.flatMap(p => p.b4jDependsOn || [])
  ]);
  const nativeB4iDependsOn = uniqueStrings([
    ...splitCsvDirective(manifest.projectb4idependson || manifest['b4i.projectdependson'] || ''),
    ...projects.flatMap(p => p.b4iDependsOn || [])
  ]);
...
function shouldAddBANanoFallbackTypes(project) {
  if (project && /banano/i.test(String(project.platform || ''))) return true;
  const deps = [];
  if (project) {
    deps.push(...(project.dependsOn || []), ...(project.b4jDependsOn || []), ...(project.b4xLibDependsOn || []), ...(project.b4xppLibDependsOn || []));
    deps.push(...(project.b4xLibB4JDependsOn || []), ...(project.b4xppLibB4JDependsOn || []));
  }
  return deps.some(dep => /^banano(?:skeleton)?$/i.test(String(dep || '').trim()));
}

function addBANanoFallbackTypesToIndex(libs, types) {
  const method = (name, params = [], returnType = '') => ({ name, params, returnType });
  const param = (name, type = 'Object') => ({ name, type });
  const prop = (name, type = 'Object') => ({ name, type });
  const lib = { name: 'BANano fallback', path: 'built-in BANano fallback', kind: 'fallback', classes: [] };
  const addClass = (name, methods = [], properties = []) => {
    const key = String(name || '').toLowerCase();
    if (!key || types.has(key)) return;
    const cls = { name, shortName: name, fullName: name, kind: 'Class', methods, properties, library: lib };
    lib.classes.push(cls);
    types.set(key, cls);
  };
  addClass('BANano', [
    method('Initialize', [param('CompilerName', 'String'), param('AppName', 'String'), param('Version', 'Int')]),
    method('Build', [param('Dir', 'String')]),
    method('Await', [param('Promise', 'BANanoPromise')], 'Object'),
    method('Alert', [param('Message', 'String')]),
    method('GetFileAsText', [param('FileURL', 'String'), param('Options', 'BANanoFetchOptions'), param('Encoding', 'String')], 'BANanoPromise'),
    method('GetFileAsDataURL', [param('FileURL', 'String'), param('Options', 'BANanoFetchOptions')], 'BANanoPromise')
  ], [prop('Header', 'BANanoHeader'), prop('TranspilerOptions', 'BANanoTranspilerOptions'), prop('JAVASCRIPT_NAME', 'String')]);
  addClass('BANanoElement', [method('Initialize', [param('Selector', 'String')]), method('Append', [param('Html', 'String')]), method('SetHTML', [param('Html', 'String')]), method('GetHTML', [], 'String')]);
  addClass('BANanoEvent');
  addClass('BANanoPromise', [method('Then', [param('Result', 'Object')]), method('Else', [param('Error', 'Object')]), method('Finally'), method('End')]);
  addClass('BANanoFetchOptions');
  addClass('BANanoObject');
  addClass('BANanoHeader', [], [prop('Title', 'String')]);
  addClass('BANanoTranspilerOptions', [], [prop('MergeAllCSSFiles', 'Boolean'), prop('MergeAllJavascriptFiles', 'Boolean'), prop('RemoveDeadCode', 'Boolean'), prop('ShowWarningDeadCode', 'Boolean')]);
  addClass('SKButton', [method('Initialize', [param('Callback', 'Object'), param('EventName', 'String'), param('ID', 'String')]), method('AddToParent', [param('ParentID', 'String')])], [prop('Text', 'String'), prop('Flavor', 'String')]);
  addClass('SKTools', [method('WriteTheme')]);
  addClass('Form');
  libs.set('banano fallback', lib);
  libs.set('banano', lib);
  libs.set('bananoskeleton', lib);
}
...
function buildB4XLibraryIndex(options, programInfo, diagnostics) {
  ...
  if (!index) {
    ...
    if (shouldAddBANanoFallbackTypes(project)) addBANanoFallbackTypesToIndex(libs, types);
  ...
  const commonDepends = project ? [...(project.dependsOn || []), ...(project.b4xLibDependsOn || []), ...(project.b4xppLibDependsOn || [])] : [];
  const platformDepends = [];
  if (project) {
    if (platformKeys.includes('b4j')) platformDepends.push(...(project.b4jDependsOn || []), ...(project.b4xLibB4JDependsOn || []), ...(project.b4xppLibB4JDependsOn || []));
    if (platformKeys.includes('b4a')) platformDepends.push(...(project.b4aDependsOn || []), ...(project.b4xLibB4ADependsOn || []), ...(project.b4xppLibB4ADependsOn || []));
    if (platformKeys.includes('b4i')) platformDepends.push(...(project.b4iDependsOn || []), ...(project.b4xLibB4iDependsOn || []), ...(project.b4xppLibB4iDependsOn || []));
  }
  ...
  for (const dep of required) {
    if (!out.libs.has(dep) && !out.types.has(dep) && !B4XPP_BUILTIN_TYPES.has(dep) && !programInfo.staticCodes.has(dep) && !programInfo.classes.has(dep)) {
      ...
      diagnostics.push(makeDiagnostic((project && project.sourcePath) || '', (project && project.line) || 1, severity, `B4X library / B4XPPLib metadata not found for dependency '${dep}'. Add its folder to the active platform library settings (${platformKeys.map(k => `b4xpp.${k}.internalLibraryDirs / b4xpp.${k}.additionalLibraryDirs`).join(', ')}) for stronger validation.`));
    }
  }
  return out;
}
...
function b4xppInferExpressionType(expr, env) {
  ...
  const eNoStrings = splitB4XStringSegments(e).map(seg => seg.inString ? '""' : seg.text).join('');
  ...
  if (/[+\-*\/]/.test(eNoStrings)) return 'Float';
  return '';
}
...
function b4xppFindKnownTypeName(programInfo, name) {
  const key = String(name || '').trim().toLowerCase();
  if (!key || !programInfo) return '';
  if (programInfo.classes && programInfo.classes.has(key)) return (programInfo.classes.get(key) || {}).name || name;
  if (programInfo.interfaces && programInfo.interfaces.has(key)) return (programInfo.interfaces.get(key) || {}).name || name;
  if (programInfo.staticCodes && programInfo.staticCodes.has(key)) return (programInfo.staticCodes.get(key) || {}).name || name;
  if (programInfo.libraryIndex && programInfo.libraryIndex.types && programInfo.libraryIndex.types.has(key)) {
    const t = programInfo.libraryIndex.types.get(key) || {};
    return t.shortName || t.name || t.fullName || name;
  }
  return '';
}

function b4xppWarnVariableTypeNameCollision(name, type, sourcePath, lineNo, diagnostics, programInfo, options) {
  if (!diagnostics || !name) return;
  const key = String(name || '').trim().toLowerCase();
  if (!key) return;
  const knownType = b4xppFindKnownTypeName(programInfo, key);
  if (!knownType) return;
  const declaredType = String(type || '').replace(/^Poly\s+/i, '').replace(/\(\)$/, '').trim();
  const typeSuffix = declaredType ? ` declared As ${declaredType}` : '';
  diagnostics.push(makeDiagnostic(sourcePath, lineNo, b4xppSemanticSeverity(options), `Variable '${name}'${typeSuffix} has the same name as type/module '${knownType}' (B4X is case-insensitive). Use a different variable name such as ${name}Instance.`));
}
...
function b4xppValidateLineSemantics(rawLine, env, sourcePath, lineNo, diagnostics, programInfo, options) {
  if (/\'\s*ignore\b/i.test(String(rawLine || ''))) return;
  ...
  if (fieldOrLocal) {
    ...
    b4xppWarnVariableTypeNameCollision(name, type, sourcePath, lineNo, diagnostics, programInfo, options);
...
function b4xppSourceLineForBlock(block, index) {
  if (block && Array.isArray(block.lineNumbers) && typeof block.lineNumbers[index] === 'number') return block.lineNumbers[index];
  // Blocks that start after a #Class / #StaticCode / #Interface directive store inner lines after the directive.
  if (block && (block.kind === 'class' || block.kind === 'static' || block.kind === 'interface')) return (block.startLine || 1) + index + 1;
  return (block && block.startLine ? block.startLine : 1) + index;
}
...
function validateStrictSemanticProgram(parsedFiles, programInfo, diagnostics, options = {}) {
  ...
  for (const parsed of parsedFiles || []) {
    ...
    if (parsed.topLevelLines && parsed.topLevelLines.some(l => String(l || '').trim())) blocks.push({ name: parsed.mainModuleName || 'Main', kind: 'main', lines: parsed.topLevelLines, lineNumbers: parsed.topLevelLineNumbers || [], sourcePath: parsed.sourcePath, startLine: 1, methods: [], globalSymbols: collectTopLevelProcessGlobals(parsed.topLevelLines) });
  }
  for (const block of blocks) {
    ...
    for (let i = 0; i < (block.lines || []).length; i++) {
      ...
      const sourceLine = b4xppSourceLineForBlock(block, i);
...
function transformOopLine(rawLine, context) {
  ...
  if (methodSig) {
    ...
    const effectiveReturnType = methodSig.modifiers.includes('async') ? (methodSig.returnType ? 'ResumableSub' : '') : methodSig.returnType;
    ...
    if (methodSig.modifiers.includes('abstract')) {
      ...
      const abstractLine = buildMethodSignatureLine(indent, abstractVisibility, generatedName, methodSig.paramsRaw, effectiveReturnType);
    ...
    if (hasB4XPPModifier || hasExplicitVisibility) {
      ...
      const generatedName = generatedMethodNameForContext(context, methodSig);
      line = buildMethodSignatureLine(indent, outVisibility, generatedName, methodSig.paramsRaw, effectiveReturnType);
    } else {
      const generatedName = generatedMethodNameForContext(context, methodSig);
      if (generatedName && generatedName.toLowerCase() !== methodSig.name.toLowerCase()) {
        line = buildMethodSignatureLine(indent, methodSig.visibility || '', generatedName, methodSig.paramsRaw, effectiveReturnType);
      }
    }
  }
...
function classHasZeroArgInitializer(programInfo, typeName) {
  if (!programInfo || !typeName) return false;
  const cls = programInfo.getClass(typeName);
  if (!cls) return false;
  const chain = [cls].concat(programInfo.ancestorChain ? programInfo.ancestorChain(typeName) : []);
  let foundInitialize = false;
  for (const item of chain) {
    for (const method of item.methods || []) {
      if (String(method.name || '').toLowerCase() !== 'initialize') continue;
      foundInitialize = true;
      if (!method.params || method.params.length === 0) return true;
    }
  }
  // B4X++ emits a default parameterless Initialize for generated classes that do not define one.
  return !foundInitialize;
}

function isAutoInitDeclaration(decl, programInfo) {
  if (!decl || !decl.varName || !decl.typeName || !programInfo) return false;
  if (!programInfo.getClass(decl.typeName)) return false;
  return classHasZeroArgInitializer(programInfo, decl.typeName);
}

function createAutoInitTracker() {
  return new Map();
}

function updateAutoInitDeclaration(line, autoInitTracker, context) {
  if (!autoInitTracker || !context || !context.programInfo) return;
  const decl = parseRegularDeclaration(line);
  if (!isAutoInitDeclaration(decl, context.programInfo)) return;
  const tailCode = splitCodeAndComment(decl.tail || '').code;
  autoInitTracker.set(decl.varName.toLowerCase(), {
    name: decl.varName,
    typeName: decl.typeName,
    initialized: /=/.test(tailCode)
  });
}

function markAutoInitAssignments(line, autoInitTracker) {
  if (!autoInitTracker || autoInitTracker.size === 0) return;
  const parts = splitCodeAndComment(line);
  const segments = splitB4XStringSegments(parts.code).filter(seg => !seg.inString).map(seg => seg.text).join(' ');
  const assignRe = /\b([A-Za-z_][A-Za-z0-9_]*)\s*=\s*([^:\r\n]+)/g;
  let m;
  while ((m = assignRe.exec(segments)) !== null) {
    const key = m[1].toLowerCase();
    const state = autoInitTracker.get(key);
    if (!state) continue;
    const value = String(m[2] || '').trim();
    state.initialized = !/^Null\b/i.test(value);
  }
}

function markExplicitInitializers(line, autoInitTracker) {
  if (!autoInitTracker || autoInitTracker.size === 0) return;
  const parts = splitCodeAndComment(line);
  const code = parts.code;
  const segments = splitB4XStringSegments(code).filter(seg => !seg.inString).map(seg => seg.text).join(' ');
  const re = /\b([A-Za-z_][A-Za-z0-9_]*)\.Initialize\s*(?:\(|$|:|')/ig;
  let m;
  while ((m = re.exec(segments)) !== null) {
    const state = autoInitTracker.get(m[1].toLowerCase());
    if (state) state.initialized = true;
  }
}

function isAutoInitTransformIgnoredLine(line) {
  const trimmed = String(line || '').trim();
  if (!trimmed || trimmed.startsWith("'")) return true;
  if (/^#/.test(trimmed)) return true;
  if (/^(?:Public\s+|Private\s+|Protected\s+)?Sub\b/i.test(trimmed)) return true;
  if (/^End\s+Sub\b/i.test(trimmed)) return true;
  if (/^(?:Sub\s+)?Class_Globals\b/i.test(trimmed)) return true;
  if (/^End\s+Class\b/i.test(trimmed)) return true;
  return false;
}

function collectAutoInitInsertionsForLine(line, autoInitTracker, context) {
  if (!autoInitTracker || autoInitTracker.size === 0 || !context || !context.programInfo) return [];
  if (isAutoInitTransformIgnoredLine(line)) return [];
  if (parseRegularDeclaration(line)) return [];

  const parts = splitCodeAndComment(line);
  const segments = splitB4XStringSegments(parts.code).filter(seg => !seg.inString).map(seg => seg.text).join(' ');
  const receiverRe = /\b([A-Za-z_][A-Za-z0-9_]*)\.([A-Za-z_][A-Za-z0-9_]*)\b/g;
  const insertions = [];
  const alreadyInserted = new Set();
  let m;
  while ((m = receiverRe.exec(segments)) !== null) {
    const varName = m[1];
    const member = m[2];
    const key = varName.toLowerCase();
    const state = autoInitTracker.get(key);
    if (!state || state.initialized || alreadyInserted.has(key)) continue;
    const memberLower = String(member || '').toLowerCase();
    if (memberLower === 'initialize' || memberLower === 'isinitialized') continue;
    if (!classHasZeroArgInitializer(context.programInfo, state.typeName)) continue;
    const indent = (String(line || '').match(/^\s*/) || [''])[0];
    insertions.push(`${indent}' B4X++ auto-init ${state.name} As ${state.typeName}`);
    insertions.push(`${indent}${state.name}.Initialize`);
...
function updateAutoInitTrackerAfterLine(line, autoInitTracker, context) {
  updateAutoInitDeclaration(line, autoInitTracker, context);
  markExplicitInitializers(line, autoInitTracker);
  markAutoInitAssignments(line, autoInitTracker);
}

function classNeedsDefaultInitializer(block) {
  if (!block) return false;
  const methods = block.methods || extractMethods(block.lines || []);
  return !(methods || []).some(m => String(m.name || '').toLowerCase() === 'initialize');
}
...
function parseClosureParams(paramsRaw) {
  return splitArguments(paramsRaw || '').map(p => p.trim()).filter(Boolean).map((p) => {
...
function parseClosureLiteralStart(line) {
  ...
  return { indent: m[1] || '', keyword: m[2], varName: m[3], declaredType: m[4], paramsRaw, params: parseClosureParams(paramsRaw), returnType: (m[6] || '').trim(), assignmentOnly: false };
}

function parseClosureAssignmentStart(line) {
  const m = String(line || '').match(/^(\s*)([A-Za-z_][A-Za-z0-9_]*)\s*=\s*Sub\s*(?:\((.*)\))?\s*(?:As\s+(.+?))?\s*$/i);
  if (!m) return null;
  const paramsRaw = (m[3] || '').trim();
  return { indent: m[1] || '', keyword: '', varName: m[2], declaredType: 'Closure', paramsRaw, params: parseClosureParams(paramsRaw), returnType: (m[4] || '').trim(), assignmentOnly: true };
}
...
function extractClosureLiterals(lines, block, diagnostics, usesRuntime) {
  ...
  for (let i = 0; i < (lines || []).length; i++) {
    ...
    const info = parseClosureLiteralStart(raw) || parseClosureAssignmentStart(raw);
    if (!info) {
      out.push(transformClosureTypeDeclarationLine(raw));
    ...
    const escapes = !!info.assignmentOnly || closureEscapes(lines, info.varName, i, endIndex) || !/^Dim$/i.test(info.keyword);
...
function isAsyncMethodSignature(sig) {
  return !!(sig && sig.modifiers && sig.modifiers.some(m => String(m).toLowerCase() === 'async'));
}

function transformAsyncSignatureLine(line, sig) {
  const indent = (String(line || '').match(/^\s*/) || [''])[0];
  let outVisibility = sig.visibility || '';
  if (outVisibility === 'protected') outVisibility = 'private';
  return buildMethodSignatureLine(indent, outVisibility, sig.name, sig.paramsRaw, sig.returnType ? 'ResumableSub' : '');
}

function stripAwaitKeyword(expr) {
  return String(expr || '').replace(/^\s*Await\b\s*/i, '').trim();
}


function hasBareAwaitKeyword(code) {
  return /(^|[^A-Za-z0-9_.])Await\b/i.test(String(code || ''));
}

function makeAwaitTempName(state) {
  state.awaitCounter = (state.awaitCounter || 0) + 1;
  return `B4XPP_await_${state.awaitCounter}`;
}

function transformAwaitLine(line, state, block, diagnostics, lineIndex) {
  const parts = splitCodeAndComment(line);
  const code = parts.code;
  const comment = parts.comment || '';
  if (!hasBareAwaitKeyword(code)) return { lines: [line], transformed: false };
  const indent = (String(code || '').match(/^\s*/) || [''])[0];

  if (!state || !state.async) {
    diagnostics && diagnostics.push(makeDiagnostic(block.sourcePath, block.startLine + (Number.isInteger(lineIndex) ? lineIndex : 0) + 1, 'error', 'Await can only be used inside an Async Sub.'));
    return { lines: [line], transformed: false };
  }

  // Dim value As Type = Await SomeResumableSub(...)
  let m = code.match(/^(\s*)Dim\s+([A-Za-z_][A-Za-z0-9_]*)(\(\))?\s+As\s+(.+?)\s*=\s*Await\s+(.+?)\s*$/i);
  if (m) {
    const varName = m[2];
    const arraySuffix = m[3] || '';
    const typeName = String(m[4] || 'Object').trim();
    const expr = stripAwaitKeyword(m[5]);
    const completeType = arraySuffix ? `${varName}() As ${typeName}` : `${varName} As ${typeName}`;
    return { lines: [`${m[1]}Wait For (${expr}) Complete (${completeType})${comment}`], transformed: true };
  }

  // target = Await SomeResumableSub(...)
  m = code.match(/^(\s*)((?:[A-Za-z_][A-Za-z0-9_]*)(?:\.[A-Za-z_][A-Za-z0-9_]*)?)\s*=\s*Await\s+(.+?)\s*$/i);
  if (m) {
    const target = m[2];
    const expr = stripAwaitKeyword(m[3]);
    const temp = makeAwaitTempName(state);
    let typeName = 'Object';
    const simpleTarget = target.match(/^[A-Za-z_][A-Za-z0-9_]*$/) ? target.toLowerCase() : '';
    if (simpleTarget && state.localTypes && state.localTypes.has(simpleTarget)) typeName = state.localTypes.get(simpleTarget).typeName || 'Object';
    else if (state.returnType) typeName = 'Object';
    return { lines: [`${m[1]}Wait For (${expr}) Complete (${temp} As ${typeName})${comment}`, `${m[1]}${target} = ${temp}`], transformed: true };
  }

  // Return Await SomeResumableSub(...)
  m = code.match(/^(\s*)Return\s+Await\s+(.+?)\s*$/i);
  if (m) {
    const expr = stripAwaitKeyword(m[2]);
    const temp = makeAwaitTempName(state);
    const typeName = state.returnType || 'Object';
    return { lines: [`${m[1]}Wait For (${expr}) Complete (${temp} As ${typeName})${comment}`, `${m[1]}Return ${temp}`], transformed: true };
  }

  // Await SomeResumableSub(...) as a statement. Kept useful for Async subs whose result is intentionally ignored.
  m = code.match(/^(\s*)Await\s+(.+?)\s*$/i);
  if (m) {
    const expr = stripAwaitKeyword(m[2]);
    const temp = makeAwaitTempName(state);
    return { lines: [`${m[1]}Wait For (${expr}) Complete (${temp} As Object)${comment}`], transformed: true };
  }

  diagnostics && diagnostics.push(makeDiagnostic(block.sourcePath, block.startLine + ((state && Number.isInteger(state.lineIndex)) ? state.lineIndex : (Number.isInteger(lineIndex) ? lineIndex : 0)) + 1, 'error', 'Unsupported Await expression. MVP supports: Dim x As T = Await ..., x = Await ..., Return Await ..., or Await ... as a standalone statement.'));
  return { lines: [line], transformed: false };
}

function transformAsyncAwaitLines(lines, block, diagnostics, programInfo) {
  const out = [];
  let state = null;
  ...
  for (let i = 0; i < (lines || []).length; i++) {
    ...
    const sig = parseMethodSignatureLine(raw);
    if (sig) {
      const isAsync = isAsyncMethodSignature(sig);
      state = { async: isAsync, name: sig.name, returnType: sig.returnType || (isAsync ? 'Object' : ''), awaitCounter: 0, localTypes, lineIndex: i };
      if (isAsync) {
        const indent = (String(raw || '').match(/^\s*/) || [''])[0];
        const asyncNote = sig.returnType ? `generated As ResumableSub` : `generated with native resumable semantics`;
        out.push(`${indent}' B4X++ Async Sub ${sig.name}${sig.returnType ? ' As ' + sig.returnType : ''}: ${asyncNote}.`);
        out.push(transformAsyncSignatureLine(raw, sig));
        continue;
      }
    }

    if (state) state.lineIndex = i;
    const transformed = transformAwaitLine(raw, state, block, diagnostics, i);
    out.push(...transformed.lines);

    if (/^\s*End\s+Sub\b/i.test(String(raw || ''))) state = null;
  }
  return out;
}

function sanitizeResidualAsyncAwaitContent(content, sourcePath = '') {
  if (!/\b(?:Async|Await)\b/i.test(String(content || ''))) return content;
  const lines = splitLines(content);
  const out = [];
  let state = null;
  const localTypes = analyzeLocalVarTypes(lines);
  const dummyBlock = { sourcePath, startLine: 0 };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (sig) {
      ...
      state = {
        async: true,
        name: sig.name,
        returnType: sig.returnType || (isAsync ? 'Object' : ''),
        awaitCounter: 0,
        localTypes,
        lineIndex: i
      };
    ...
    if (state) state.lineIndex = i;
    if (hasBareAwaitKeyword(splitCodeAndComment(raw).code)) {
      const effectiveState = state || { async: true, name: '', returnType: 'Object', awaitCounter: 0, localTypes, lineIndex: i };
      const transformed = transformAwaitLine(raw, effectiveState, dummyBlock, null, i);
      out.push(...transformed.lines);
    } else {
      out.push(raw);
    }
...
function sanitizeResidualAsyncAwaitOutputs(outputs) {
  return (outputs || []).map(out => {
    if (!out || typeof out.content !== 'string') return out;
    const content = sanitizeResidualAsyncAwaitContent(out.content, out.sourcePath || '');
...
function transformBodyLines(lines, block, diagnostics, programInfo, usesRuntime) {
  lines = transformAsyncAwaitLines(lines, block, diagnostics, programInfo);
  ...
  const autoInitTracker = createAutoInitTracker();
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const autoInitLines = collectAutoInitInsertionsForLine(lines[i], autoInitTracker, context);
    if (autoInitLines.length) out.push(...autoInitLines);
    out.push(...transformOopLine(lines[i], context));
    updateAutoInitTrackerAfterLine(lines[i], autoInitTracker, context);
  }
  const hasResidualAwait = out.some(line => hasBareAwaitKeyword(splitCodeAndComment(line).code));
  return hasResidualAwait ? transformAsyncAwaitLines(out, block, diagnostics, programInfo) : out;
}
...
function flattenClassParts(block, options, programInfo, diagnostics, usesRuntime) {
  ...
  for (let idx = 0; idx < chain.length; idx++) {
    ...
    const fields = [];
    const asyncExpandedLines = transformAsyncAwaitLines(bundle.lines, owner, diagnostics, programInfo);
    const closureExpandedBundle = extractClosureLiterals(asyncExpandedLines, owner, diagnostics, usesRuntime);
    ...
    for (let i = 0; i < bodySourceLines.length; i++) {
      ...
      const autoInitLines = collectAutoInitInsertionsForLine(bodySourceLines[i], autoInitTracker, context);
      if (autoInitLines.length) transformed.push(...autoInitLines);
      transformed.push(...transformOopLine(bodySourceLines[i], context));
      updateAutoInitTrackerAfterLine(bodySourceLines[i], autoInitTracker, context);
    }
...
function sanitizeMethodParameterCollisions(body, globalNames, diagnostics, sourcePath, baseLine, unsafeNames) {
  ...
  for (let i = 0; i < out.length; i++) {
    ...
    for (let paramIndex = 0; paramIndex < sig.params.length; paramIndex++) {
      ...
      // Be conservative here: common B4X source names such as Name, Value or Distance
      // are valid as parameters even when a property/type/member with the same name
      // exists.  Rename only when the generated flattened code would really collide
      // with a generated global field or with a reserved keyword. Variable/type name
      // collisions are reported separately without rewriting user parameters.
      if (!lname || (!hidesGeneratedGlobal && !isReservedWord)) continue;
      const combinedUnsafe = new Set([...(globalNames || new Set())]);
      const safeName = makeSafeParameterName(param.name, combinedUnsafe);
      replacements.set(lname, safeName);
      if (diagnostics) {
        let reason = 'a generated global field';
        if (isReservedWord) reason = 'a reserved B4X keyword';
...
function transpileClass(block, options = {}, programInfo, usesRuntime) {
  ...
  if (classNeedsDefaultInitializer(block)) {
    lines.push("' B4X++ default initializer for generated class modules.");
    lines.push('Public Sub Initialize');
    lines.push('End Sub');
    lines.push('');
  }
...
function sanitizeGenericNamePart(value) {
  let text = String(value || '').trim();
  text = text.replace(/^Poly\s+/i, '').replace(/\(\)$/g, '');
...
function makeGenericSpecializationName(baseName, args) {
  const readable = `${baseName}__${(args || []).map(sanitizeGenericNamePart).join('__')}`;
  if (readable.length <= 80 && IDENTIFIER_RE.test(readable)) return readable;
  return `${baseName}__G${b4xppShortHash(`${baseName}<${(args || []).join(',')}>`)}`;
}

function parseGenericOfExpressionAt(text, index) {
  const source = String(text || '');
  const nameMatch = source.slice(index).match(/^([A-Za-z_][A-Za-z0-9_]*)\b/);
  if (!nameMatch) return null;
  const baseName = nameMatch[1];
  let cursor = index + nameMatch[0].length;
  while (cursor < source.length && /\s/.test(source[cursor])) cursor++;
  if (source[cursor] !== '(') return null;
  const afterOpen = source.slice(cursor + 1);
  if (!/^\s*Of\b/i.test(afterOpen)) return null;
  const balanced = readBalancedParentheses(source, cursor);
  if (!balanced) return null;
  const inner = String(balanced.inner || '').trim();
  const ofMatch = inner.match(/^Of\s+([\s\S]+)$/i);
  if (!ofMatch) return null;
  const args = splitArguments(ofMatch[1]).map(a => a.trim()).filter(Boolean);
  if (!args.length) return null;
  return { baseName, args, start: index, end: balanced.end + 1, raw: source.slice(index, balanced.end + 1) };
}

function rewriteGenericTypeUsagesInCode(code, genericTemplates, diagnostics, sourcePath, lineNo, collector) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    const expr = parseGenericOfExpressionAt(text, i);
    if (!expr) continue;
    const template = genericTemplates.get(expr.baseName.toLowerCase());
    if (!template) continue;
    if ((template.genericParams || []).length !== expr.args.length) {
      diagnostics.push(makeDiagnostic(sourcePath, lineNo, 'error', `Generic type ${expr.baseName} expects ${(template.genericParams || []).length} argument(s), found ${expr.args.length}.`));
      continue;
    }
    const rewrittenArgs = expr.args.map(arg => rewriteGenericTypeUsagesInLine(arg, genericTemplates, diagnostics, sourcePath, lineNo, collector).trim());
    const generatedName = makeGenericSpecializationName(template.name, rewrittenArgs);
    if (collector) collector({ templateName: template.name, typeArgs: rewrittenArgs, generatedName, sourcePath, lineNo });
    out += text.slice(cursor, i) + generatedName;
    cursor = expr.end;
    i = expr.end - 1;
  }
...
function rewriteGenericTypeUsagesInLine(line, genericTemplates, diagnostics, sourcePath, lineNo, collector) {
  const parts = splitCodeAndComment(line);
  const segments = splitB4XStringSegments(parts.code).map(seg => {
    if (seg.inString) return seg.text;
    return rewriteGenericTypeUsagesInCode(seg.text, genericTemplates, diagnostics, sourcePath, lineNo, collector);
  }).join('');
  return segments + parts.comment;
}

function replaceGenericParamsInCode(code, typeMap) {
  const replacements = typeMap || new Map();
  if (!replacements.size) return code;
  return String(code || '').replace(/\b[A-Za-z_][A-Za-z0-9_]*\b/g, (word, offset, whole) => {
    const before = whole.slice(Math.max(0, offset - 10), offset);
    const after = whole.slice(offset + word.length, offset + word.length + 10);
    const key = word.toLowerCase();
    if (!replacements.has(key)) return word;
    // Replace only places that look like type positions. This keeps normal variables named T from being touched.
    if (/\bAs\s+$/i.test(before) || /\bPoly\s+$/i.test(before) || /\bOf\s+$/i.test(before) || /^\s*(?:,|\)|\()/i.test(after) || /^\s*(?:=|$)/.test(after)) {
...
function replaceGenericParamsInLine(line, typeMap) {
  const parts = splitCodeAndComment(line);
  const segments = splitB4XStringSegments(parts.code).map(seg => seg.inString ? seg.text : replaceGenericParamsInCode(seg.text, typeMap)).join('');
  return segments + parts.comment;
}

function genericUsageKey(templateName, typeArgs) {
  return `${String(templateName || '').toLowerCase()}<${(typeArgs || []).map(a => String(a || '').toLowerCase()).join(',')}>`;
}

function collectGenericTemplates(parsedFiles) {
  const templates = new Map();
  const owners = new Map();
  for (const parsed of parsedFiles || []) {
    for (const cls of parsed.classes || []) {
      if (!cls.genericParams || cls.genericParams.length === 0) continue;
      const key = cls.name.toLowerCase();
      if (!templates.has(key)) {
        templates.set(key, cls);
        owners.set(key, parsed);
      }
    }
  }
...
function collectGenericUsagesFromLines(lines, genericTemplates, diagnostics, sourcePath, startLine, usageMap) {
  for (let i = 0; i < (lines || []).length; i++) {
    rewriteGenericTypeUsagesInLine(lines[i], genericTemplates, diagnostics, sourcePath, (startLine || 0) + i + 1, (usage) => {
      const key = genericUsageKey(usage.templateName, usage.typeArgs);
      if (!usageMap.has(key)) usageMap.set(key, usage);
    });
  }
}

function instantiateGenericClass(template, typeArgs, genericTemplates, diagnostics, usage) {
  const typeMap = new Map();
  for (let i = 0; i < (template.genericParams || []).length; i++) {
    typeMap.set(String(template.genericParams[i] || '').toLowerCase(), typeArgs[i] || 'Object');
  }
  const generatedName = usage.generatedName || makeGenericSpecializationName(template.name, typeArgs);
  const lines = (template.lines || []).map((line, idx) => {
    let out = replaceGenericParamsInLine(line, typeMap);
    out = rewriteGenericTypeUsagesInLine(out, genericTemplates, diagnostics, template.sourcePath, template.startLine + idx + 1, null);
    return out;
  });
  const extendsName = template.extendsName && typeMap.has(template.extendsName.toLowerCase()) ? typeMap.get(template.extendsName.toLowerCase()) : template.extendsName;
  const implementsNames = (template.implementsNames || []).map(n => typeMap.get(String(n || '').toLowerCase()) || n).filter(Boolean);
  return {
    ...template,
    name: generatedName,
    genericParams: [],
    genericSpecialization: { templateName: template.name, typeArgs: typeArgs.slice(), generatedName },
    extendsName,
    implementsNames,
    lines,
...
function cloneParsedForGenericExpansion(parsed) {
  return {
    ...parsed,
    classes: (parsed.classes || []).filter(cls => !(cls.genericParams && cls.genericParams.length)).map(cls => ({ ...cls, lines: (cls.lines || []).slice(), methods: (cls.methods || []).slice() })),
    staticCodes: (parsed.staticCodes || []).map(mod => ({ ...mod, lines: (mod.lines || []).slice(), methods: (mod.methods || []).slice() })),
    interfaces: (parsed.interfaces || []).map(intf => ({ ...intf, lines: (intf.lines || []).slice(), methods: (intf.methods || []).slice() })),
    topLevelLines: (parsed.topLevelLines || []).slice()
  };
}

function expandGenericSpecializations(parsedFiles, diagnostics) {
  const { templates, owners } = collectGenericTemplates(parsedFiles);
  if (!templates.size) return parsedFiles;

  const usageMap = new Map();
  for (const parsed of parsedFiles || []) {
    collectGenericUsagesFromLines(parsed.topLevelLines || [], templates, diagnostics, parsed.sourcePath, 0, usageMap);
    for (const cls of parsed.classes || []) collectGenericUsagesFromLines(cls.lines || [], templates, diagnostics, cls.sourcePath, cls.startLine, usageMap);
    for (const mod of parsed.staticCodes || []) collectGenericUsagesFromLines(mod.lines || [], templates, diagnostics, mod.sourcePath, mod.startLine, usageMap);
    for (const intf of parsed.interfaces || []) collectGenericUsagesFromLines(intf.lines || [], templates, diagnostics, intf.sourcePath, intf.startLine, usageMap);
  }
  ...
  while (changed && guard++ < 50) {
    changed = false;
    for (const usage of Array.from(usageMap.values())) {
      const key = genericUsageKey(usage.templateName, usage.typeArgs);
      if (specializations.has(key)) continue;
      const template = templates.get(String(usage.templateName || '').toLowerCase());
      if (!template) continue;
      const cls = instantiateGenericClass(template, usage.typeArgs, templates, diagnostics, usage);
      specializations.set(key, cls);
      changed = true;
      collectGenericUsagesFromLines(cls.lines || [], templates, diagnostics, cls.sourcePath, cls.startLine, usageMap);
    }
  }
  if (guard >= 50) diagnostics.push(makeDiagnostic((parsedFiles[0] && parsedFiles[0].sourcePath) || '', 1, 'error', 'Generic specialization recursion limit reached. Check recursive generic type usages.'));

  const expanded = (parsedFiles || []).map(cloneParsedForGenericExpansion);
  for (const parsed of expanded) {
    parsed.topLevelLines = parsed.topLevelLines.map((line, i) => rewriteGenericTypeUsagesInLine(line, templates, diagnostics, parsed.sourcePath, i + 1, null));
    for (const cls of parsed.classes || []) {
      cls.lines = cls.lines.map((line, i) => rewriteGenericTypeUsagesInLine(line, templates, diagnostics, cls.sourcePath, cls.startLine + i + 1, null));
      cls.methods = extractMethods(cls.lines || []);
    }
    for (const mod of parsed.staticCodes || []) {
      mod.lines = mod.lines.map((line, i) => rewriteGenericTypeUsagesInLine(line, templates, diagnostics, mod.sourcePath, mod.startLine + i + 1, null));
      mod.methods = extractMethods(mod.lines || []);
    }
    for (const intf of parsed.interfaces || []) {
      intf.lines = intf.lines.map((line, i) => rewriteGenericTypeUsagesInLine(line, templates, diagnostics, intf.sourcePath, intf.startLine + i + 1, null));
      intf.methods = extractMethods(intf.lines || []);
    }
  }

  const firstOwnerByKey = new Map();
  for (const [key, template] of templates) firstOwnerByKey.set(key, owners.get(key) || expanded[0]);
  for (const cls of specializations.values()) {
    const owner = firstOwnerByKey.get(String(cls.genericSpecialization.templateName || '').toLowerCase()) || expanded[0];
    const outParsed = expanded.find(p => p.sourcePath === owner.sourcePath) || expanded[0];
    outParsed.classes.push(cls);
  }
  return expanded;
}
...
function transpileParsedFiles(parsedFiles, options = {}) {
  ...
  parsedFiles = expandGenericSpecializations(parsedFiles, diagnostics);
  ...
  for (const parsed of parsedFiles) {
    ...
    if (parsed.fromB4XPPLib) continue;
  ...
  // Safety net: a closure can be generated from several syntaxes, including
  // split declarations such as:
  //   Dim c As Closure
  //   c = Sub (...) ... End Sub
  // If any output references B4XPPClosure, the runtime class module must be
  // emitted and later registered as a normal B4X module. This avoids producing
  // a .bas project with "Dim c As B4XPPClosure" but no B4XPPClosure.bas.
  const outputNeedsClosureRuntime = outputs.some(o => /\bB4XPPClosure\b/i.test(String(o && o.content || '')));
  const outputHasClosureRuntime = outputs.some(o => String(o && o.moduleName || '').toLowerCase() === 'b4xppclosure');
  if ((usesRuntime.usesClosure || outputNeedsClosureRuntime) && !outputHasClosureRuntime) {
    outputs.push(makeClosureModule(options, usesRuntime.maxClosureArgs));
  }

  const sanitizedOutputs = sanitizeResidualAsyncAwaitOutputs(outputs);
...
function collectNativeProjectDependencyNames(project, platformKeys) {
  const names = [];
  if (!project) return names;
  names.push(...(project.dependsOn || []));
  const keys = platformKeys || [];
  if (keys.includes('b4j')) names.push(...(project.b4jDependsOn || []));
  if (keys.includes('b4a')) names.push(...(project.b4aDependsOn || []));
  if (keys.includes('b4i')) names.push(...(project.b4iDependsOn || []));
  return uniqueStrings(names).filter(Boolean);
}

function collectB4XPPLibDependencyNames(project, options) {
  const names = [];
  if (!project) return names;
  names.push(...(project.b4xppLibDependsOn || []));
  const keys = projectPlatformKeys(project, options || {});
  if (keys.includes('b4j')) names.push(...(project.b4xppLibB4JDependsOn || []));
  if (keys.includes('b4a')) names.push(...(project.b4xppLibB4ADependsOn || []));
  if (keys.includes('b4i')) names.push(...(project.b4xppLibB4iDependsOn || []));
  return uniqueStrings(names).filter(Boolean);
}

function collectB4XPPLibDependencyNamesFromLib(lib, platformKeys) {
  const names = [];
  if (!lib) return names;
  names.push(...(lib.dependsOn || []));
  if ((platformKeys || []).includes('b4j')) names.push(...(lib.b4jDependsOn || []));
  if ((platformKeys || []).includes('b4a')) names.push(...(lib.b4aDependsOn || []));
  if ((platformKeys || []).includes('b4i')) names.push(...(lib.b4iDependsOn || []));
  return uniqueStrings(names).filter(Boolean);
}

function collectLibraryDirsForProjects(options, projects) {
  const dirs = [];
  if (projects && projects.length) {
    for (const project of projects) dirs.push(...collectLibraryDirs(options || {}, project));
  } else {
    dirs.push(...collectLibraryDirs(options || {}, null));
  }
  return uniqueStrings(dirs).filter(Boolean);
}

function scanB4XPPLibPackages(dirs) {
  ...
  for (const dir of dirs || []) {
    ...
    for (const file of entries) {
      ...
      try {
        ...
        const manifestEntry = entries.find(e => /(^|\/)manifest\.txt$/i.test(e.name));
        if (manifestEntry) {
          const manifest = parseKeyValueManifest(manifestEntry.data.toString('utf8'));
          if (manifest.name) name = manifest.name;
        }
      } catch {}
      packages.set(String(name || '').toLowerCase(), { name, path: full });
    }
  }
  return packages;
}

function resolveB4XPPLibDependencies(parsedFiles, options, diagnostics) {
  const rootProjects = (parsedFiles || []).map(p => p.project).filter(Boolean);
  const dirs = collectLibraryDirsForProjects(options || {}, rootProjects);
  if (!dirs.length) return { parsedFiles: [], libraries: [] };
  const available = scanB4XPPLibPackages(dirs);
  const platformKeys = rootProjects.length ? uniqueStrings(rootProjects.flatMap(p => projectPlatformKeys(p, options || {}))) : projectPlatformKeys(null, options || {});
  const queue = [];
  for (const project of rootProjects) {
    queue.push(...collectB4XPPLibDependencyNames(project, options || {}));
    // Migration / safety net: if a .b4xpplib name was accidentally declared through
    // #ProjectDependsOn / #ProjectB4JDependsOn, treat it as a B4X++ source package.
    // It will still be filtered out from native B4X IDE LibraryN entries by the extension.
    for (const dep of collectNativeProjectDependencyNames(project, platformKeys)) {
      if (available.has(String(dep || '').toLowerCase())) queue.push(dep);
    }
  }
  ...
  while (queue.length) {
    ...
    if (!item) {
      if (diagnostics) diagnostics.push(makeDiagnostic((rootProjects[0] && rootProjects[0].sourcePath) || '', (rootProjects[0] && rootProjects[0].line) || 1, 'warning', `B4XPPLib dependency not found: '${dep}'. Add the .b4xpplib file to the active platform library folders.`));
      continue;
    }
    try {
      const lib = parseB4XPPLibFile(item.path);
      resolved.push(lib);
      for (const d of lib.diagnostics || []) if (diagnostics) diagnostics.push(d);
      for (const parsed of lib.parsedFiles || []) resolvedParsed.push(parsed);
      for (const nextDep of collectB4XPPLibDependencyNamesFromLib(lib, platformKeys)) {
        if (!seen.has(String(nextDep || '').toLowerCase())) queue.push(nextDep);
      }
    } catch (err) {
      if (diagnostics) diagnostics.push(makeDiagnostic((rootProjects[0] && rootProjects[0].sourcePath) || '', (rootProjects[0] && rootProjects[0].line) || 1, 'warning', `Could not parse B4XPPLib '${dep}': ${err.message}`));
    }
  }
  return { parsedFiles: resolvedParsed, libraries: resolved };
}
...
function transpileFiles(files, options = {}) {
  ...
  const packageResolution = resolveB4XPPLibDependencies(parsedFiles, options, packageDiagnostics);
  parsedFiles.push(...(packageResolution.parsedFiles || []));

  const result = transpileParsedFiles(parsedFiles, options);
  result.diagnostics.unshift(...packageDiagnostics);
  result.b4xpplibDependencies = packageResolution.libraries || [];
  ...
  return {
    ...
    b4xpplibDependencies: packageResolution.libraries || [],
    diagnosticsByPath
  };
}
