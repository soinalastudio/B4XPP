...
const B4XPP_GENERATOR_VERSION = '0.2';
...
function transformOopLine(rawLine, context) {
  ...
  if (methodSig) {
    ...
    if (methodSig.modifiers.includes('abstract')) {
      ...
      const abstractLine = buildMethodSignatureLine(indent, abstractVisibility, methodSig.name, methodSig.paramsRaw, methodSig.returnType);
    ...
    if (hasB4XPPModifier || hasExplicitVisibility) {
      ...
      line = buildMethodSignatureLine(indent, outVisibility, methodSig.name, methodSig.paramsRaw, methodSig.returnType);
    }
  }
