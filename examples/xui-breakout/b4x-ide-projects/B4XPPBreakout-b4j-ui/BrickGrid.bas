...
Public Sub CheckBallCollision(aGameBall As Ball) As Int
    ...
    For brickIndex = 0 To mBricks.Size - 1
        ...
        If brickItem.getBroken = False And aGameBall.CollidesWithBox(brickItem.Left, brickItem.Top, brickItem.Right, brickItem.Bottom) Then
            ...
            If gainedPoints > 0 Then setRemaining(getRemaining - 1)
            Return gainedPoints
        End If
    Next
    Return 0
...
' B4X++ property: public Remaining As Int default=0
Public Sub getRemaining As Int
	Return mRemaining
End Sub

Public Sub setRemaining(B4XPP_Remaining As Int)
	mRemaining = B4XPP_Remaining
End Sub
