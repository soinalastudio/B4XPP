...
B4X++ generated properties are declared with bare `Property`. Do not use `#Property` for B4X++ generated getters/setters: `#Property` is a native B4X directive and must be preserved as-is in generated `.bas` output.
...
## v0.4.3 ResumableSub navigation notes

- Go to Definition should resolve `Wait For(SomeSub(...)) Complete (...)` and `Dim rs As ResumableSub = SomeSub(...)` back to the `.bx` Sub declaration.
- B4X resumable Subs are identified by `As ResumableSub`, or by containing `Wait For` / `Sleep`.
- Keep this as editor navigation only; do not reimplement B4X's resumable state machine in B4X++ output. The native B4X compiler keeps ownership of actual `Wait For` / `Sleep` behavior.


## v0.4.5 Async wrappers

B4XPP.Core includes B4XPPAsync and Task(Of T). B4XPP.Net includes B4XPPHttp and B4XPPHttpResponse and propagates platform HTTP library dependencies to generated B4X project files.
